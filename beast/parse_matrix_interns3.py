...
parrot.add_argument('-xml', type=str, help=".xml to pull region (& country) states and priors from.")
...
root = xml.getroot()												#get the list of regions (and/or countrys), in order, from the xml
traits = root.findall('generalDataType')
country_list = None
for trait in traits:
	if trait.get('id').startswith('country'):
		country_list = [ state.get('code') for state in trait ]
		n_countrys = len(country_list)
...
print region_list
if country_list:
	print 'found these %d countrys:'%len(country_list)
	print country_list
...
for i in country_list:
	for j in country_list:
		if i == j:
			...
			if 'country.rates.%s.%s'%(i,j) in log_data.columns.values:
				log_data['country.actualRates.%s.%s'%(i,j)] = log_data['country.rates.%s.%s'%(i,j)]*log_data['country.indicators.%s.%s'%(i,j)]
			else:
				continue
...
def find_bf(indicator_avg, prior_expectation, n_demes, len_chain=50000000): # Demes == 'categories' (e.g., regions, or country species)
...
region_actualrates_name = "%s_country_AR.png"%file_stem
...
## If we have a country trait, too, then do the same for this data.
...
if country_list:
	...
	#convert indicators to bayes factors while we're at it
	country_bf_series = pd.Series([ find_bf(series['avg'], n_countrys-1, n_countrys) for column, series in log_data.iteritems() if 'country.indicator' in column ]) #we will then use this to fill our matrices.

	country_actualRates = fill_matrix(country_actualRates, country_actualRates_series)				#fill our matrices
	country_bf = fill_matrix(country_bf, pd.Series(country_bf_series))
	...
	country_bf_name = "%s_country_bf.png"%file_stem													# Make pretty heat map figures.
	country_actualrates_name = "%s_country_AR.png"%file_stem
	country_map_name = "%s_country_network.png"%file_stem

	make_heatmap(country_actualRates, 'country Transition Rates', country_actualrates_name)
	make_heatmap(country_bf, 'Bayes Factors for country Transitions', country_bf_name)
