...
root = xml.getroot()												#get the list of regions (and/or countries), in order, from the xml
...
for i in range(1, (n_countries+1)**2):
	if 'country.rates%d'%i in log_data.columns.values:
		log_data['country.actualRates%d'%i] = log_data['country.rates%d'%i]*log_data['country.indicators%d'%i]
	else:
		continue
...
def find_bf(indicator_avg, prior_expectation, n_demes, len_chain=50000000): # Demes == 'categories' (e.g., regions, or countries)
